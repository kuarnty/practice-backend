...
class UserValidationService(
    ...
    fun validateUserForCreate(createUserInput: CreateUserInput): Mono<ValidationResult> {
    ...
    fun validateUserForUpdate(updateUserInput: UpdateUserInput): Mono<ValidationResult> {
        ...
        // val nameCheck = if (name != null) userRepository.existsByNameAndIdNot(name, currentId) else Mono.just(false)
        // val emailCheck = if (email != null) userRepository.existsByEmailAndIdNot(email, currentId) else Mono.just(false)
        // return nameCheck.flatMap { nameConflict ->
        //     if (nameConflict) {
        //         Mono.just(ValidationResult.fail("Name already exists.", mapOf("name" to "exists"), "NAME_EXISTS"))
        //     } else {
        //         emailCheck.map { emailConflict ->
