...
class LectureService(
    ...
    fun createLecture(title: String?, description: String?, instructor: String): Mono<Lecture> {
        return lectureValidationService.validateLectureForCreate(title, description, instructor)
            .flatMap { vr ->
    ...
    fun updateLecture(id: String, title: String?, description: String?, instructor: String?): Mono<Lecture> {
        return lectureValidationService.validateLectureForUpdate(id, title, description, instructor)
            .flatMap { vr ->
                if (!vr.isValid) return@flatMap Mono.error(IllegalArgumentException(vr.errorMessage ?: "validation failed"))
